...
public class EffectiveAmbienceSounds {
	...
	protected static SoundEvent create(String name) {
		SoundEvent soundEvent = SoundEvent.createVariableRangeEvent(Effective.id(name));
		SOUND_EVENTS.add(soundEvent);
		return soundEvent;
	}

	public static void initialize() {
		SOUND_EVENTS.forEach(soundEvent -> Registry.register(BuiltInRegistries.SOUND_EVENT, soundEvent.getLocation(), soundEvent));
	}
}
