...
public class SplashParticle extends Particle {
	...
	protected SplashParticle(ClientLevel world, double x, double y, double z) {
		super(world, x, y, z);
		...
		this.waveRimModel = new SplashRimModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(SplashRimModel.MODEL_LAYER));
		this.waveBottomRimModel = new SplashBottomRimModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(SplashBottomRimModel.MODEL_LAYER));
		this.gravity = 0.0F;
	...
	private void drawSplash(int frame, Camera camera, float tickDelta, Vector3f multiplier) {
		...
		this.waveRimModel.renderToBuffer(modelRimMatrix, rimModelConsumer, light, OverlayTexture.NO_OVERLAY, 1.0f, 1.0f, 1.0f, 1.0f);
		this.waveBottomRimModel.renderToBuffer(modelRimBottomMatrix, rimModelConsumer, light, OverlayTexture.NO_OVERLAY, 1.0f, 1.0f, 1.0f, 1.0f);
