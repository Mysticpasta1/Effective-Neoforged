...
public class Effective implements ClientModInitializer {
	...
	// sound events
	public static SoundEvent AMBIENCE_WATERFALL = SoundEvent.createVariableRangeEvent(Effective.id("ambience.waterfall"));
	public static SoundEvent PARRY = SoundEvent.createVariableRangeEvent(Effective.id("entity.parry"));
	private static int ticksJeb;

	public static boolean isNightTime(Level world) {
		return world.getSunAngle(world.getDayTime()) >= 0.25965086 && world.getSunAngle(world.getDayTime()) <= 0.7403491;
	}
...
//	}
	...
	public void onInitializeClient() {
		...
		EntityModelLayerRegistry.registerModelLayer(SplashModel.MODEL_LAYER, SplashModel::getLayerDefinition);
		EntityModelLayerRegistry.registerModelLayer(SplashBottomModel.MODEL_LAYER, SplashBottomModel::getLayerDefinition);
		EntityModelLayerRegistry.registerModelLayer(SplashRimModel.MODEL_LAYER, SplashRimModel::getLayerDefinition);
		EntityModelLayerRegistry.registerModelLayer(SplashBottomRimModel.MODEL_LAYER, SplashBottomRimModel::getLayerDefinition);

		// particles
		SPLASH = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("splash"), new SplashParticleType(true));
		ParticleFactoryRegistry.getInstance().register(SPLASH, SplashParticle.DefaultFactory::new);
		DROPLET = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("droplet"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(DROPLET, DropletParticle.DefaultFactory::new);
		RIPPLE = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("ripple"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(RIPPLE, RippleParticle.DefaultFactory::new);
		GLOW_SPLASH = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("glow_splash"), new SplashParticleType(true));
		ParticleFactoryRegistry.getInstance().register(GLOW_SPLASH, GlowSplashParticle.DefaultFactory::new);
		GLOW_DROPLET = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("glow_droplet"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(GLOW_DROPLET, GlowDropletParticle.DefaultFactory::new);
		GLOW_RIPPLE = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("glow_ripple"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(GLOW_RIPPLE, GlowRippleParticle.DefaultFactory::new);
		ALLAY_TWINKLE = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("allay_twinkle"), new AllayTwinkleParticleType());
		ParticleFactoryRegistry.getInstance().register(ALLAY_TWINKLE, AllayTwinkleParticleType.Factory::new);
		CHORUS_PETAL = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("chorus_petal"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(CHORUS_PETAL, ChorusPetalParticle.DefaultFactory::new);
		EYES = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("eyes"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(EYES, EyesParticle.DefaultFactory::new);
		WILL_O_WISP = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("will_o_wisp"), FabricParticleTypes.simple(true));
		ParticleFactoryRegistry.getInstance().register(WILL_O_WISP, fabricSpriteSet -> new WillOWispParticle.DefaultFactory(fabricSpriteSet, Effective.id("textures/entity/will_o_wisp.png"), 0.1f, 0.75f, 1.0f, 0.0f, 0.1f, 1.0f));

		// lodestone particles
		ParticleFactoryRegistry.getInstance().register(PIXEL, LodestoneWorldParticleType.Factory::new);
		PIXEL = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("pixel"), PIXEL);
		ParticleFactoryRegistry.getInstance().register(WISP, LodestoneWorldParticleType.Factory::new);
		WISP = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("wisp"), WISP);
		ParticleFactoryRegistry.getInstance().register(FLAME, FlameParticleType.Factory::new);
		FLAME = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("flame"), FLAME);
		ParticleFactoryRegistry.getInstance().register(DRAGON_BREATH, FlameParticleType.Factory::new);
		DRAGON_BREATH = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("dragon_breath"), DRAGON_BREATH);
		ParticleFactoryRegistry.getInstance().register(BUBBLE, BubbleParticleType.Factory::new);
		BUBBLE = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("bubble"), BUBBLE);
		ParticleFactoryRegistry.getInstance().register(WATERFALL_CLOUD, WaterfallCloudParticleType.Factory::new);
		WATERFALL_CLOUD = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("waterfall_cloud"), WATERFALL_CLOUD);
		ParticleFactoryRegistry.getInstance().register(MIST, MistParticleType.Factory::new);
		MIST = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("mist"), MIST);
		ParticleFactoryRegistry.getInstance().register(FIREFLY, FireflyParticleType.Factory::new);
		FIREFLY = Registry.register(BuiltInRegistries.PARTICLE_TYPE, Effective.id("firefly"), FIREFLY);

		// sound events
		AMBIENCE_WATERFALL = Registry.register(BuiltInRegistries.SOUND_EVENT, AMBIENCE_WATERFALL.getLocation(), AMBIENCE_WATERFALL);
		PARRY = Registry.register(BuiltInRegistries.SOUND_EVENT, PARRY.getLocation(), PARRY);
		...
		ShaderEffectRenderCallback.EVENT.register(tickDelta -> {
			if (EffectiveConfig.shouldGlowSquidsHypnotize() && (!RenderedHypnotizingEntities.GLOWSQUIDS.isEmpty() || RenderedHypnotizingEntities.lockedIntensityTimer > 0 || RenderedHypnotizingEntities.lookIntensity > 0)) {
				...
				GlowSquid bestSquid = null;

				for (GlowSquid glowsquid : RenderedHypnotizingEntities.GLOWSQUIDS) {
					if (glowsquid.position().distanceToSqr(Minecraft.getInstance().player.position()) <= 49f) {
						Vec3 toSquid = glowsquid.position().subtract(Minecraft.getInstance().player.position()).normalize();
						double lookIntensity = toSquid.dot(Minecraft.getInstance().player.getViewVector(tickDelta)); // * 1 / Math.max(2, Math.sqrt(glowsquid.getPos().squaredDistanceTo(MinecraftClient.getInstance().player.getPos())) - 5f); // 1 = looking straight at squid
						if (lookIntensity > bestLookIntensity && Minecraft.getInstance().player.hasLineOfSight(glowsquid)) {
				...
				if (!Minecraft.getInstance().isPaused() && RenderedHypnotizingEntities.lockedIntensityTimer >= 0) {
				...
				RenderedHypnotizingEntities.lookIntensity = Mth.clamp(RenderedHypnotizingEntities.lookIntensity, 0, 0.5f);
				...
				sTimeHypno.set((Minecraft.getInstance().level.getGameTime() + tickDelta) / 20);
				HYPNO_SHADER.render(tickDelta);

				if (Minecraft.getInstance().player.tickCount % 20 == 0 && !Minecraft.getInstance().isPaused()) {
					Minecraft.getInstance().player.playSound(SoundEvents.GLOW_SQUID_AMBIENT, (float) RenderedHypnotizingEntities.lookIntensity, (float) RenderedHypnotizingEntities.lookIntensity);
				}

				// look at squid
				if (EffectiveConfig.glowSquidHypnotize == EffectiveConfig.GlowSquidHypnoOptions.ATTRACT && bestSquid != null && !Minecraft.getInstance().isPaused()) {
					LocalPlayer player = Minecraft.getInstance().player;

					Vec3 target = bestSquid.position();
					Vec3 vec3d = player.position();
					double d = target.x - vec3d.x;
					double e = target.y - vec3d.y - 1;
					double f = target.z - vec3d.z;
					double g = Math.sqrt(d * d + f * f);
					float currentPitch = Mth.wrapDegrees(player.getViewXRot(tickDelta));
					float currentYaw = Mth.wrapDegrees(player.getViewYRot(tickDelta));
					...
					Vec2 rotationChange = new Vec2(
						Mth.wrapDegrees(desiredPitch - currentPitch),
						Mth.wrapDegrees(desiredYaw - currentYaw)
					);

					Vec2 rotationStep = rotationChange.normalized().scale((float) RenderedHypnotizingEntities.lookIntensity * 10f * (Mth.clamp(rotationChange.length(), 0, 10) / 10f));

					player.setXRot(player.getViewXRot(tickDelta) + rotationStep.x);
					player.setYRot(player.getViewYRot(tickDelta) + rotationStep.y);
				}
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			if (freezeFrames > 0) {
				...
				boolean bl = client.hasSingleplayerServer() && !client.getSingleplayerServer().isDedicatedServer(); //TODO: Check if correct.
				if (bl) {
